...
pub(crate) fn sync_scroll_physics_from_tree(
    element: &sniffer_core::types::Element,
    ...
    match element {
        sniffer_core::types::Element::Container { children, .. }
        | sniffer_core::types::Element::ScrollView { children, .. }
        | sniffer_core::types::Element::SharedView { children, .. } => {
...
pub(crate) fn inject_physics_to_tree(
    element: &mut sniffer_core::types::Element,
    target_id: u64,
    pos_x: f32,
    pos_y: f32,
) {
    if let sniffer_core::types::Element::ScrollView { id, scroll_x, scroll_y, .. } = element {
        if let Some(id_str) = id {
            if sniffer_core::ui::widget::fnv1a(id_str.as_bytes()) == target_id {
                *scroll_x = pos_x;
                *scroll_y = pos_y;
                return;
            }
...
pub(crate) fn update_indicator_dots_in_element(
    element: &mut sniffer_core::types::Element,
    active_page: i32,
    vmin_px: f32,
) -> bool {
    if let sniffer_core::types::Element::Container { id, children, style } = element {
        if let Some(id_str) = id {
            if id_str == "page_indicator_container" {
                let is_landscape = style.flex_direction == sniffer_core::style::FlexDirection::Column;
                let mut changed = false;
                for (p, child_el) in children.iter_mut().enumerate() {
                    let is_active = (p as i32) == active_page;
                    if let sniffer_core::types::Element::Container { style: dot_style, .. } = child_el {
                        ...
                        if is_landscape {
                            let new_h = sniffer_core::style::Dimension::Pixels(if is_active { 3.6 * vmin_px } else { 1.6 * vmin_px });
                            let new_w = sniffer_core::style::Dimension::Pixels(1.6 * vmin_px);
                            ...
                            let new_w = sniffer_core::style::Dimension::Pixels(if is_active { 3.6 * vmin_px } else { 1.6 * vmin_px });
                            let new_h = sniffer_core::style::Dimension::Pixels(1.6 * vmin_px);
    ...
    } else if let sniffer_core::types::Element::ScrollView { children, .. } = element {
        for child in children {
            if update_indicator_dots_in_element(child, active_page, vmin_px) {
                return true;
            }
