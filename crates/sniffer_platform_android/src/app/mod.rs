    ...
    while state.running {
        ...
        if needs_redraw {
            ...
            if screen_changed {
                ...
                state.cached_safe_area = crate::jni::get_safe_area(&app)
                    .map(|(top, bottom)| {
                        (
            ...
            let is_first_frame = matches!(&root_element, sniffer_core::types::Element::Container { children, .. } if children.is_empty());
        ...
        let min_frame_time = Duration::from_micros(1000);
