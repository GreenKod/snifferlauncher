                ...
                MotionAction::Up | MotionAction::PointerUp => {
                    ...
                    if is_static_tap {
                        ...
                        if let Some((clicked_btn, rect)) = clicked {
                            sniffer_core::dev_log!("[Input] TAP on button id: {clicked_btn} (hex: {clicked_btn:x})");
                            state
                                .event_bus
                                .push(UiEvent::Click(clicked_btn, rect.width, rect.height));
                        } else {
                            sniffer_core::dev_log!("[Input] TAP clicked outside (no button at start={:?}, end={:?})", state.touch_start_pos, point);
                            state.event_bus.push(UiEvent::ClickOutside);
                        }
                    } else {
                        sniffer_core::dev_log!("[Input] Swipe detected (dist={tap_dist:.1}, thresh={tap_threshold:.1})");
                        state.event_bus.push(UiEvent::ClickOutside);
                    }
                    InputStatus::Handled
                }
