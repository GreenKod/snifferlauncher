            ...
            match motion_event.action() {
                MotionAction::Down | MotionAction::Move | MotionAction::PointerDown => {
                    ...
                    let gesture_name = match motion_event.action() {
                        ...
                        MotionAction::Move => {
                            let dist = (point.x - state.touch_start_pos.x)
                                .hypot(point.y - state.touch_start_pos.y);
                            if dist > 16.0 * state.cached_density.0.max(1.0) {
                                "SWIPE / DRAG".to_string()
                            } else {
                                "HOLD".to_string()
                            }
                        }
                ...
                MotionAction::Up | MotionAction::PointerUp => {
                    ...
                    let tap_dist = (point.x - state.touch_start_pos.x)
                        .hypot(point.y - state.touch_start_pos.y);
                    ...
                    if is_static_tap {
                        ...
                        if let Some((clicked_btn, rect)) = clicked {
                            ...
                            state.event_bus.push(UiEvent::Click(
                                clicked_btn,
                                rect.width,
                                rect.height,
                            ));
