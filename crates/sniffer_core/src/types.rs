            ...
            match property {
                ...
                "width" => {
                    if let Ok(w) = value.parse::<f32>() {
                        style.width = crate::style::Dimension::Pixels(w);
                    } else if value == "auto" {
                        style.width = crate::style::Dimension::Auto;
                    } else if let Some(p) = value.strip_suffix('%')
                        && let Ok(pct) = p.parse::<f32>()
                    {
                        style.width = crate::style::Dimension::Percent(pct);
                    }
                }
                "height" => {
                    if let Ok(h) = value.parse::<f32>() {
                        style.height = crate::style::Dimension::Pixels(h);
                    } else if value == "auto" {
                        style.height = crate::style::Dimension::Auto;
                    } else if let Some(p) = value.strip_suffix('%')
                        && let Ok(pct) = p.parse::<f32>()
                    {
